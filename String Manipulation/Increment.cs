...
namespace String_Manipulation
{
	internal sealed class Increment
	{
		...
		private Increment(AsyncPackage package, OleMenuCommandService commandService)
		{
		...
		public static Increment Instance { private set; get; }
		...
		public static async Task InitializeAsync(AsyncPackage package)
		{
			...
			Instance = new Increment(package, commandService);
		}
		...
		private async void Execute(object sender, EventArgs e)
		{
			...
			text = Regex.Replace(text, @"-?(\d+\.\d+|\d+)", match =>
			{
				...
				var increment = Math.Pow(10, exponent).ToDecimal();
				number += increment;
